...
## 36. 喷火/排气管口修正 + 排气喷气效果(2026-09-13, kart.html)
- **排气口 port0**: 官方端口表(deob L14035: port0/port1=排气, fire0..5=喷火口, balloon 等)。
  cotton1 的 kart.gltf 只有 port0 节点, matrix 平移 (0.198, 0.822, 0.231) = ReKart 数据空间;
  **数据空间 +Y 是车尾**(zup_root 为 -90°X 旋转: 数据+Y→世界-Z)。旧代码误把 y<0 当车尾,
  火焰出现在车头且朝上, 已修正: 双锥火焰(外橙 0.075×0.55 / 内黄 0.04×0.35)挂在 port0,
  锥尖朝数据 +Y(水平向后), 仅 boost 时可见并闪烁。
- **排气喷气(常开)**: 官方 ReExhaustEffect(deob L8170-8301)参数全量移植:
  发射间隔 max(0, 120-kmh*0.3) ms; 排气初速 5 u/s, 方向=车局部(±0.1,1,±0.1)归一化(≈水平向后带抖动);
  粒子同时继承车速项 |v|(0.9/s 衰减)→ 被甩在车后; alpha 125/255, 衰减 0.75/s(寿命~167ms);
  尺寸 0.13×scale, scale+7/s; 相机 billboard; 上限 10 粒子; additive。
  贴图 effect/exhaust/exhaust.png 本地未解包, 用 canvas 径向渐变软圆代替(alpha×2.2 补偿)。
- 验证: 浏览器实测 —— 21km/h 喷火截图确认火焰从排气管口水平向后;
  排气粒子满 10 个常开运转。
...
## 41. 集气速度/漂移衰减/气槽清零(2026-09-13, 用户判定后修正)
- **集气过快根因**: 我原实现用 drive.speed²(车速)当官方公式的 localRightSpeed²(侧向速度),
  高速下 0.1s 即满。官方慢速分支(L24365-24368, driftMaxGauge!=1): s = vl²·dt,
  权重 前0.2s×3 / 0.5s内×1.5 / 之后÷(2·elapsed)。
  **证据说明**: 本客户端 kartspec 全部 2836 行 driftMaxGauge=1 → 官方 p3528 行为是
  "1 次漂移=1 条槽", 不存在 5-6 次的参数; 慢速手感是经典跑跑行为, 经用户判定采用
  官方慢速公式 + 标度 k=0.0008(强漂移单次≈0.2, 5-8 次集满, 实测单次 0.23)。
- **气槽清零**: commit 时 gauge=0 且 pending=0(L24381), 满槽转 N2O 入槽(L23726)后
  集气条 visibly 归零 —— 实测漂移后条归零、槽位亮 N2O。
- **漂移速度衰减(用户判定缺失)**: 官方由侧滑摩擦持续耗能涌现; 矢量模型中 vl 抓地衰减
  天然等效; 旧标量路径补显式式 speed -= sign·v·ω²/grip·dt(grip=9×driftSlipFactor=1.8)。
  实测: 172km/h 进入漂移 0.6s 后 126km/h。
- **普通转弯拖拽**: cornerDrawFactor=0.18(kartspec, L24251) 沿 -Forward 的回正拖拽
  0.18·|v·ω| 已实现(两路径)。
- **小喷时机**: B50 尾窗现覆盖"轻点漂移"(触发期结束即入 decay 也开窗 0.5s)。
