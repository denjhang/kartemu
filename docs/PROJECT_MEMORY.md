...
**kart.html 可玩版迭代中**(2026-09-14): 真实官方路线地图 + 2D 矢量轮胎物理 + 漂移/集气/氮气槽/
大喷连喷 + 官方 cotton 速度表 + 引擎音效已落地(实现溯源见 UNDERSTOOD §35)。
用户要求: 每个功能必须溯源到反编译代码, 用数据说话; 做完一个验证一个, 不许堆半成品。
已知待办: BGM/音乐、比赛流程 UI、道具系统(官方 H5 无对抗道具, 见 FUNCTIONS_DETAIL §G.9)。
