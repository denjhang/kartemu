...
**编写函数级交接文档,换台式机继续** —— 已完成(2026-09-11):
新建 `docs/FUNCTIONS.md`(按 §A-§L 12 模块组织,以函数为单位记录签名/参数/返回值/调用关系/用法)。
- ✅ §A 物理引擎 / §B 碰撞系统 / §C 赛道构建 / §D 比赛流程+UI:核心函数已详述
- 📌 §E 输入 / §F 音效 / §G 道具 / §H AI:已索引关键函数清单,待按行号补签名
- ⏳ §I 渲染 / §J 资源解析 / §K 数学库 / §L 顶层函数索引:列清单,待台式机继续

**下一步(台式机)**:按 `FUNCTIONS.md` 中 📌 / ⏳ 标记的行号,在 `deob_named.js`
中补齐剩余函数签名,优先物理引擎/碰撞/赛道实现。
...
4. 广告牌背面实心橙(cull 方向/alpha)。
5. 碰撞重做(官方 Z$ 4m 均匀网格 rayQuery + 墙障碍, §26 已逆向, 待实现)。
6. 物理引擎实现(§25 已逆向, 待用 JS 重新实现: stepSubstep/applyLongitudinal/applySteeringAndTires)。
7. 比赛流程实现(§29 已逆向: raceLifecycle 状态机 + HUD + 相机)。
8. 可选:官方 body 贴图合成(Fw/Dw,§20.6 已完整逆向算法);车辆轮子动画。
9. 把 index.html 的骑乘人物换成新的骨骼蒙皮版(挂到赛车子树时去掉 zup_root 旋转)。
10. [已完成] KartRider-Tools v1.2.2 解包(§21)。
11. (台式机继续)补齐 FUNCTIONS.md 中 📌 / ⏳ 标记的函数签名,按行号在 deob_named.js 查找。
