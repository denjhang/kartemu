...
## 2026-09-13 续3: 漂移两阶段物理(官方 L24205-24242 忠实实现)
- **trigger/active(按住 Shift)= 冰面**: 官方漂移轮胎力 ×0.2×tireEnvelope, envelope 以
  1e-6/子步生长 ≈ 0 → 侧向抓地≈0: 横摆角速度保持(黄油滑)、velDir 冻结、几乎不掉速
  (仅 0.15m/s² 滚阻)。velDir 硬跟随 lerp 已整体删除(非漂移由动态力模型积分 velDir)。
- **松 Shift(decay)= 抓地回归**: 正常动态轮胎力模型接管(4900N/滑移饱和0.10rad),
  侧向速度分量被真实回收 → 10-20km/h 衰减自然涌现; 反打方向 δ<0 加速回收("拉回车头")。
- driftingNow 语义收窄: 仅 trigger/active; decay 归正常力模型。
- 实测: 直道172km/h → 冰面漂移1s 179km/h(不掉速); 松开后集气 commit 入槽、条清零。
